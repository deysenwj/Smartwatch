  ...
  async function handleUpdate(id: string, status: Report["status"], note: string) {
    ...
    if (hasSupabaseConfig()) {
      try {
        ...
        if (updated) {
          try {
            const verb = status === "Selesai" ? "disetujui" : status === "Ditolak" ? "ditolak" : status === "Prioritas" ? "ditandai sebagai prioritas" : "sedang diproses";
